...
void cli_add_error(cli_error_buffer* errors, const cli_error_category category, const enum cli_argument_type arg_type, const char char8, const char* str, const int32_t int32)
{
...
void cli_remove_error(cli_error_buffer* errors, const cli_error_category category, const enum cli_argument_type arg_type, const char char8, const char* str)
{
...
static int cli_qsort_error_comp(const void* lhs, const void* rhs)
{
    ...
    return (int)CLI_ERROR_KEY_CATEGORY(lhs_error->key) - (int)CLI_ERROR_KEY_CATEGORY(rhs_error->key);
}

int cli_default_error_report(const char* program_name, cli_formatter* formatter, cli_error_buffer* errors)
{
    ...
    qsort(errors->data, errors->count, sizeof(const cli_error), cli_qsort_error_comp);
    ...
    for (int i = 0; i < errors->count; ++i)
    {
        ...
        switch (category)
        {
            case CLI_ERROR_CATEGORY_MISSING_REQUIRED_ARGUMENT:
            {
                ...
                {
                    cli_fmt(formatter, "%s: error: the following arguments are required: ", program_name);
                }
            ...
            {
                cli_fmt(formatter, "%s: error: option ", program_name);
                cli_fmt_put_option_name(formatter, error->char8, error->str);
...
cli_status cli_parse_command(const int argc, char* const* argv, cli_parser* parser)
{
    ...
    // setup parser
    cli_error_buffer* errors = parser->error_buffer;
    ...
    while (nargs_parsed < argc)
    {
        ...
        switch (token.type)
        {
            ...
            case CLI_TOKEN_LONG_OPTION:
            {
                ...
                const int min_nargs = is_n_or_more ? option_info->nargs - CLI_0_OR_MORE : option_info->nargs;
                ...
                for (int i = 0; i < CLI_MIN(max_nargs, argc_remaining); ++i)
                {
            ...
            case CLI_TOKEN_POSITIONAL:
            {
                ...
                cli_remove_error(errors, CLI_ERROR_CATEGORY_MISSING_REQUIRED_ARGUMENT, CLI_ARGUMENT_POSITIONAL, '\0', positional_name);
                break;
            }
...
CLI_POP_WARNING
    ...
    cli_status status = CLI_STATUS_COUNT;

    if (cli->error_buffer == NULL)
    {
        // default error handling
        cli_error_buffer errors = { 0 };
        status = cli_parse_command(subcommand_argc, subcommand_argv, &(cli_parser) {
        ...
        if (status == CLI_STATUS_ERROR)
        {
            char error_string[4096];
            cli_formatter formatter = { .buffer_capacity = CLI_ARRAY_SIZE(error_string), .buffer = error_string };
            cli_default_error_report(program_command->name, &formatter, &errors);
            fprintf(stderr, "%s\n", error_string);
        }
    }
    else
    ...
    else if (status == CLI_STATUS_HELP)
    {
...
cli_status cli_run(const cli_result* program)
{
    ...
    return cmd->run(&program->commands[0], cmd);
}
...
cli_status cli_run_all(const cli_result* program)
{
    ...
    {
        if (program->commands[i].run != NULL)
        {
            const cli_status status = program->commands[i].run(&program->commands[0], &program->commands[i]);
